...
/*        employeeId.add((long) 229);
    ...
    @ApiOperation(value = "根据学生ID和时间段获取该时间段该学生的授课安排表" , notes = "")
    @GetMapping(value = "/curriculum/getCurriculumID")
    @ResponseBody
    public void getCurriculumID(@RequestParam(required = false) Long studentId,
                                 @RequestParam(required = false) String start,
                                 @RequestParam(required = false) String end,
                                 HttpServletResponse response) throws Exception {
        response.setContentType("application/json;charset=utf-8");
        String json = Result.build(ResultType.Success).appendData("curriculumID",curriculumService.selectCurriculumByUserAndCanlendar(studentId,start,end)).convertIntoJSON();
        response.getWriter().write(json);
    }

    @ApiOperation(value = "根据学号或教工号+学期+星期获取课程表" , notes = "")
    @GetMapping(value = "/curriculum/getCurriculumByOne")
    @ResponseBody
    public void selectCurriculumByStuno(@RequestParam(required = false) Long no,
                                        @RequestParam(required = false) Long semesterId,
                                        @RequestParam(required = false) String week ,
                                        HttpServletResponse response) throws Exception {
        response.setContentType("application/json;charset=utf-8");
        System.out.println(no + " " + semesterId + " " + week);
        String json = Result.build(ResultType.Success).appendData("curriculum",curriculumService.getCurriculumByStudentOrEmployeeAndSemesterIDANDWeek(no,semesterId,week)).convertIntoJSON();
        response.getWriter().write(json);
    }

    @ApiOperation(value = "获取学年" , notes = "")
    @GetMapping(value = "/curriculum/getAllYear")
    @ResponseBody
    public void selectCurriculumByStuno(HttpServletResponse response) throws Exception {
        response.setContentType("application/json;charset=utf-8");
        String json = Result.build(ResultType.Success).appendData("curriculum",curriculumService.selectAllYear()).convertIntoJSON();
        System.out.println(curriculumService.selectAllYear());
        response.getWriter().write(json);
    }
    // todo 晓锋修改过，要使用或修改跟晓锋说下
    @ApiOperation(value = "获取学院信息" , notes = "")
    @GetMapping(value = "/curriculum/getInstitute")
    public void getInstituteName(HttpServletResponse response) throws IOException {
        response.setContentType("application/json;charset=utf-8");
        String json = Result.build(ResultType.Success).appendData("Institute",departmentService.selectAllInstitute()).convertIntoJSON();
        response.getWriter().write(json);
    }
}
